...
def _event(state: AgentState, event_type: str, actor: str, context: Context, payload: dict) -> LedgerEvent:
    prior=state.get("ledger_events", []); return LedgerEvent(new_id("evt"), event_type, actor, now(), context, payload, prior[-1].event_id if prior else None)
...
def build_graph(llm: LLMBackend, sandbox: Sandbox):
    ...
    def interpret_goal(state):
        ...
        criteria=[AcceptanceCriterion(criterion_id=f"criterion_{i+1}", **item) for i,item in enumerate(payload["acceptance_criteria"])]
    ...
    def execute(state):
        ...
        created=[]
        for typ,value in (("stdout",result.stdout),("stderr",result.stderr),("exit_code",result.exit_code)):
    ...
    def verify(state):
        ...
        for c in state["acceptance_criteria"]:
            ...
            else: uncertainty.append("No authorized direct evidence is available for this criterion.")
            for eid in attempt.evidence_ids:
                e=next(e for e in state["evidence"] if e.evidence_id==eid)
                if (e.evidence_type=="exit_code" and c.verification_method=="exit_code") or (e.evidence_type=="stdout" and c.verification_method=="stdout_contains"): (supported if satisfied else contradictory).append(eid)
            ...
            claims.append(Claim(new_id("claim"),f"Criterion {c.criterion_id} {'is' if satisfied else 'is not'} satisfied.","inference","reasoning_model",_context(state,attempt.code),supported+contradictory,confidence,"verified" if satisfied else "contradicted",[]))
    ...
    def constitutional(state):
        ...
        all_verified=all(r.satisfied and r.confidence>=CONFIDENCE_THRESHOLD for r in required); contradicted=any(not r.satisfied for r in required)
        decision={"status":"verified" if all_verified else ("contradicted" if contradicted else "insufficient evidence"),"claim_ids":[c.claim_id for c in state["claims"]],"evidence_ids":[e.evidence_id for e in state["evidence"]]}
    ...
    def diagnose(state):
        ...
        claim=Claim(new_id("claim"),"Execution or verification evidence indicates the current code does not meet all required criteria.","interpretation","reasoning_model",context,source,0.8,"supported")
    ...
    def repair(state):
        last=state["attempts"][-1]; context=_context(state,last.code); retained=list(state["claims"]); prompt=f"Goal: {state['normalized_goal']}\nCode:\n```python\n{last.code}\n```\nEvidence: stdout={last.stdout!r}; stderr={last.stderr!r}. Fix the code and return corrected code only."
